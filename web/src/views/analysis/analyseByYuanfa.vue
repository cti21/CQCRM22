<template>
    <div>
        <div class="filter-container">
              ...
              <!--</el-date-picker>-->
              <!--<el-button class="filter-item" style="margin-left: 20px" type="primary"  @click="handleFilter" icon="el-icon-search">搜索</el-button>-->
              <!--<el-button class="filter-item" style="margin-left: 20px"type="primary" @click="handleReset" icon="el-icon-download">重置</el-button>-->
        ...
        <div style="margin-top: 20px">
          <el-table
            ...
            <el-table-column prop="name" label="原发病类型" align="center">
            </el-table-column>
             <el-table-column prop="num" label="人次" align="center">
            </el-table-column>
          </el-table>
        </div>
    </div>
...
import { analyseByYuanfa } from '@/api/analysis'
...
export default {
  ...
  methods: {
    getAnalyseByYuanfa(data) {
      analyseByYuanfa(data).then(response => {
    ...
    handleReset() {
      ...
      this.searchEndyear = ''
      this.getAnalyseByYuanfa(this.queryparams)
    },
    handleFilter() {
      ...
      this.getAnalysePTH(this.queryparams)
    },
    exportExcel() {
      const wb = XLSX.utils.table_to_book(document.querySelector('#outTable'))
      const wbout = XLSX.write(wb, { bookType: 'xlsx', bookSST: true, type: 'array' })
      try {
        FileSaver.saveAs(new Blob([wbout], { type: 'application/octet-stream' }), '原发病统计.xlsx')
      } catch (e) {
