<template>
    <div>
        <div class="filter-container">
              <el-date-picker v-model="searchyear" class="myInput" type="month" placeholder="请输入年份" value-format="yyyy-MM">
              </el-date-picker>
...
import { txDeviceBymonth } from '@/api/analysis'
...
export default {
  ...
  methods: {
    gettxDeviceBymonth(data) {
      txDeviceBymonth(data).then(response => {
    ...
    handleReset() {
      ...
      this.searchyear = ''
      this.gettxDeviceBymonth(this.queryparams)
    },
    handleFilter() {
    ...
    exportExcel() {
      ...
      try {
        FileSaver.saveAs(new Blob([wbout], { type: 'application/octet-stream' }), '透析器统计表.xlsx')
      } catch (e) {
