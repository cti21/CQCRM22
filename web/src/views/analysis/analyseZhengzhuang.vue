<template>
    <div>
        <div class="filter-container">
              ...
              <el-date-picker v-model="startyear" style="width: 145px" class="myInput" type="date" placeholder="输入起始日期" value-format="yyyy-MM-dd">
              </el-date-picker>
              <el-date-picker v-model="endyear" style="width: 145px" class="myInput" type="date" placeholder="输入结束日期" value-format="yyyy-MM-dd">
              </el-date-picker>
...
import { analyseZhengzhuang } from '@/api/analysis'
...
export default {
  ...
  methods: {
    getAnalyseZhengzhuang(data) {
      analyseZhengzhuang(data).then(response => {
    ...
    handleReset() {
      ...
      this.reportType = []
      this.getAnalyseZhengzhuang(this.queryparams)
    },
    handleFilter() {
    ...
    exportExcel() {
      ...
      try {
        FileSaver.saveAs(new Blob([wbout], { type: 'application/octet-stream' }), '症状统计表.xlsx')
      } catch (e) {
