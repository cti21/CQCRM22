...
import { analyseMaterialByyear } from '@/api/analysis'
...
export default {
  ...
  methods: {
    getMaterialByyear(data) {
      analyseMaterialByyear(data).then(response => {
    ...
    handleReset() {
      ...
      this.searchEndyear = ''
      this.getMaterialByyear(this.queryparams)
    },
    handleFilter() {
    ...
    exportExcel() {
      ...
      try {
        FileSaver.saveAs(new Blob([wbout], { type: 'application/octet-stream' }), '患者转归报表.xlsx')
      } catch (e) {
