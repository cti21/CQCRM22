<template>
    <div>
        <div class="filter-container">
              <el-input v-model="searchName" class="myInput" style="width: 120px;margin-right: 20px" placeholder="患者姓名"></el-input>
...
import { analyseBfzBymonth } from '@/api/analysis'
...
export default {
  ...
  methods: {
    getBingfazheng(data) {
      analyseBfzBymonth(data).then(response => {
    ...
    handleReset() {
      ...
      this.searchName = ''
      this.getBingfazheng(this.queryparams)
    },
    handleFilter() {
    ...
    exportExcel() {
      ...
      try {
        FileSaver.saveAs(new Blob([wbout], { type: 'application/octet-stream' }), '急性并发症报表.xlsx')
      } catch (e) {
