<template>
    <div>
        ...
        <div>
          <el-dialog  title="绩效" width="40%" :visible.sync="dialogFormVisible">
...
<script>
import { benefit_dicts, createBenefit_dict, updateBenefit_dict, deleteBenefit_dict } from '@/api/system'
  ...
  methods: {
    ...
    createData() {
      this.$refs.dataForm.validate((valid) => {
        if (valid) {
          ...
          createBenefit_dict(this.area).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateBenefit_dict(tempData).then(() => {
            this.dialogFormVisible = false
            this.$notify({
              title: '成功',
              message: '更新成功',
    ...
    handleDelete(row) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        this.area = Object.assign({}, row) // copy obj
        const tempData = Object.assign({}, this.area)
        deleteBenefit_dict(tempData).then(() => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
