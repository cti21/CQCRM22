...
<script>
import { performance_dicts, createPerformance_dict, updatePerformance_dict, deletePerformance_dict } from '@/api/system'
  ...
  methods: {
    ...
    createData() {
      this.$refs.dataForm.validate((valid) => {
        if (valid) {
          ...
          createPerformance_dict(this.area).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updatePerformance_dict(tempData).then(() => {
            this.dialogFormVisible = false
            this.$notify({
              title: '成功',
              message: '更新成功',
    ...
    handleDelete(row) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        this.area = Object.assign({}, row) // copy obj
        const tempData = Object.assign({}, this.area)
        deletePerformance_dict(tempData).then(() => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
