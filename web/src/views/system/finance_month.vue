<template>
    <div>
        <el-row>
            <el-select v-model="searchname" clearable style="width: 200px" class="myInput" placeholder="请选择">
        ...
        <div>
          <el-dialog  title="财务月设置" width="40%" :visible.sync="dialogFormVisible">
...
<script>
import { finance_months, depts, createFinance_month, updateFinance_month, deleteFinance_month } from '@/api/system'
...
export default {
  ...
  methods: {
    getFinance_month(data) {
      finance_months(data).then(response => {
    ...
    handleReset() {
      ...
      this.listQuery.name = ''
      this.searchname = ''
      this.getFinance_month(this.listQuery)
    },
    handleCreate() {
      this.resetArea()
    ...
    createData() {
      this.$refs.dataForm.validate((valid) => {
        if (valid) {
          ...
          createFinance_month(tempData).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          const tempData = Object.assign({}, this.area)
          updateFinance_month(tempData).then(() => {
            this.dialogFormVisible = false
            this.$notify({
              title: '成功',
              message: '更新成功',
    ...
    handleDelete(row) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        this.area = Object.assign({}, row) // copy obj
        const tempData = Object.assign({}, this.area)
        deleteFinance_month(tempData).then(() => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
