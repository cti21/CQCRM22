<template>
    <div>
        ...
        <div>
          <el-dialog  title="部门" width="40%" :visible.sync="dialogFormVisible">
                     ...
                     <el-row>
                       <el-col>
                          <el-form-item label="上级分部">
                            <el-input v-model="area.subcompany" readonly class="myInput"></el-input>
                          </el-form-item>
                       </el-col>
                     </el-row>
                    <el-row>
                       <el-col>
                          <el-form-item label="上级部门">
                            <el-input v-model="area.supdepartment" readonly class="myInput"></el-input>
                          </el-form-item>
                       </el-col>
                     </el-row>
                  </el-form>
...
<script>
import { departments, createDepartment, updateDepartment, deleteDepartment } from '@/api/login'
  ...
  methods: {
    ...
    createData() {
      this.$refs.dataForm.validate((valid) => {
        if (valid) {
          ...
          const status = this.nodeOject.kind
          this.area.status = this.nodeOject.kind
          if (status === 3) {
            this.area.supdepid = this.nodeOject.code
          } else {
            this.area.supdepid = this.nodeOject.code
          }
          createDepartment(this.area).then(() => {
            this.$emit('refreshTree', {})
    ...
    handleDelete(row) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        this.area = Object.assign({}, row) // copy obj
        const tempData = Object.assign({}, this.area)
        deleteDepartment(tempData).then(() => {
          this.$emit('refreshTree', {})
          this.$notify({
            title: '成功',
            message: '删除成功',
